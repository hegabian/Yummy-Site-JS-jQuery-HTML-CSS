...
async function catMeals(){
    var meals = await fetch(`https://www.themealdb.com/api/json/v1/1/list.php?i=list`);
        ...
        for(i=0; i<responseMeals.meals.length ;i++){
            cartonaa += `
            <div class="col-md-3">
                <div onclick="ingreOfMeal('${responseMeals.meals[i].strIngredient}')" class=" card  card_ingre px-3 text-white text-center overflow-hidden bg-transparent" style="width: 18rem;">
                    <i class="fa-solid mb-2 fa-drumstick-bite card-img-top" style="font-size: 5rem;" ></i>  
                    <h3 class="mb-2">${responseMeals.meals[i].strIngredient}</h3>
                    <p class="overflow-hidden px-2">${getFirst50Words(responseMeals.meals[i].strDescription)}</p>   
                </div>
            </div>
           `
    ...
    async function ingreOfMeal(y){
        var meals = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${y}`);
            var responseMeals= await meals.json();
            var card = document.getElementById("show-cards")
            var cartonaa = "";
            for(i=0; i<responseMeals.meals.length ;i++){
